...
function AlienLaunchMarker:update(dt)
    ...
    elseif love.keyboard.wasPressed('space') and not self.alien[1].collided then

        for x = 1, 2 do
            local alien = Alien(self.world, 'round', self.alien[1].body:getX(), self.alien[1].body:getY(), 'Player')
            alien.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 10)
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        for a, alien in pairs(self.alien) do
            alien:render()
        end
    end
end