...
function GenerateQuadsFlags(atlas)
    ...
    for k = 1 , 8 do
        if k <= 4 then
            spritesheet[sheetCounter] = love.graphics.newQuad(x,y,16,48,atlas:getDimensions())
            x = x + 16
        else
            spritesheet[sheetCounter] = love.graphics.newQuad(x,y,48,16,atlas:getDimensions())
            y = y + 16
        end
